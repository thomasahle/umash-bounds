...
This is an independent derivation and implementation, not a rerun of the reviewer's programs. All enumerations ran on `<xeon>` (Xeon Platinum 8375C), in `<xeon>/umash-enh-verify`, at nice level 10 with at most 32 compute threads for this task. The Mac only edited, read, and transferred files. Certificates use integer counts and rational numbers; decimals are only explanatory. The supplied UMASH sources were not changed.
