...
def subcase_b(ds):
    rows = []
    prior = read('umash-subcase-b/verification/results/table.json')
    for h in range(1, 16):
        n, s = [0] * (1 << h), [0] * (1 << h)
        mask = (1 << h) - 1
        run = (1 << h) - 8
...
def imported_checks():
    primary = read('checks3/primary_certificate.json')
    ...
    ph_enh = read('checks2/open_ph_enh_certificate.json')
